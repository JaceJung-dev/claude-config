...
> **⚠️ 참고:** 각 Bash 도구 호출은 독립된 새 셸이므로, 아래 각 블록은 경로 해석 스니펫을 맨 앞에 재실행한다.
...
그 다음 사용자에게 "어떤 skill과 plugin을 켤까요?"라고 묻는다. 응답을 받은 뒤 아래 블록으로 실행한다(선택 안 한 플래그는 생략):
...
SCRIPT="$REPO/bin/init-project.sh"
[ -x "$SCRIPT" ] || { echo "init-project.sh 못 찾음: $SCRIPT (링크 깨짐?)"; exit 1; }
# 사용자가 고른 skill/plugin으로 실행 (선택 안 한 플래그는 생략)
"$SCRIPT" --skills "<고른 스킬>" --plugin "<고른 플러그인>"
```
