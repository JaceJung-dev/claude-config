...
- **내가 만든 스킬** — `library/skills/`에 본체 보관, 이 레포가 기준. 프로젝트별 opt-in.
- **서드파티 스킬** — `npx skills`가 글로벌 설치, 레포는 `library/skill-lock.json` 락파일만 추적.
- **플러그인** — 마켓플레이스에서 설치. 일부는 스킬을 함께 제공.

> 서드파티 스킬의 단일 기준은 `library/skill-lock.json`입니다. 추가·삭제 시 이 표도 같이 갱신하세요.
> 설치/업데이트 명령은 [README](../README.md#서드파티-스킬-npx--매니페스트) 참고.
...
> 본체는 레포에 두지 않고 `library/plugin-lock.json`(매니페스트)으로 추적합니다 — 새 머신에선
> `bin/install-plugins.sh`가 설치합니다. **설치 대상은 매니페스트, enable 여부는 위 settings.json**으로
